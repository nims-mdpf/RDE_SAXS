# RDEデータセットテンプレート　RDE_SAXS　を試してみる

RDEデータセットテンプレート `RDE_SAXS`　をローカル開発環境で動かす方法を説明します。
...
テンプレートの選択
- RDE_SAXSデータセットテンプレートは、RIGAKU製XRD装置が出力するSAXSデータras形式およびrasx形式、ならびにSAXSデータセットから出力したcsv形式に対応しています
- 利用時はSAXS、SAXS_fitting、SAXS_fitting_CSVのいずれかを選択します(両型式を同時に扱うことができません)
- templateフォルダには、SAXS用、SAXS_fitting用、SAXS_fitting_CSV用それぞれのテンプレートが収められています
...
3. containerの内容　展開した状態では以下の通り
    ```cmd
    $ ls
    Dockerfile  main.py  modules_saxs modules_tool  pip.conf  pyproject.toml  requirements-test.txt  requirements.txt  tox.ini
    ```
4. 仮想環境作成(pyenvの事例)
    ```cmd
    $ pyenv local 3.12.9
...
10. テンプレートファイルの配置
  ...
  - ここではSAXS用テンプレートを選択しています
    ```cmd
    (venv) $ cp -p  ../templates/saxs/tasksupport/* data/tasksupport/
    (venv) $ tree data
    data
    ├── inputdata
    ├── invoice
...
11. 入力データの配置
    ...
    - 入力データは各自ご用意してください
    - この説明ではtestdata-14.ras というファイルを使っています
```cmd
(venv) $ cp  ../inputdata/testdata-14.ras data/inputdata
```
12. 送状ファイルの配置
    - 送状ファイル(invoice.json)をdata/invoice以下に配置します
    - テスト用のサンプルを利用してください
    ```cmd
    (venv) $ cp  ../tryout/invoice_sample_saxs.json data/invoice/invoice.json
    ```
13. ファイル配置の確認
